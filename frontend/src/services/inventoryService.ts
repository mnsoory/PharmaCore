...
const mapDrugData = (drugData: DrugDetailResponse, batches: StockBatchResponse[]): Drug => {
  const totalAvailable = batches.reduce((acc, batch) => acc + batch.remainingQty, 0);
  const avgPurchasePrice = batches.length > 0 
...
export const inventoryService = {
    ...
    updateDrug: async (id: number, drugData: EditDrugFormData): Promise<void> => {
        await axiosClient.put(`/drugs/${id}`, drugData);
    },
  ...
  getAllDrugs: async (): Promise<Drug[]> => {
    ...
    const allBatchesRes = await axiosClient.get<StockBatchResponse[]>('/stockbatches');
