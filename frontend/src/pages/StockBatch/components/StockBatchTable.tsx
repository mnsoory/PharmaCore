...
import { ChevronUp, ChevronDown, Layers, SlidersHorizontal } from "lucide-react";
...
const statusConfig: Record<StockBatch["status"], string> = {
  ...
  "Expiring Soon":"bg-orange-50  text-orange-700  border-orange-200  dark:bg-orange-950  dark:text-orange-300  dark:border-orange-800",
  "Expired":      "bg-red-50     text-red-700     border-red-200     dark:bg-red-950     dark:text-red-300     dark:border-red-800",
};
...
const StockBatchTable: React.FC<Props> = ({ batches, sortField, sortDir, onSort, onAdjust }) => {
            ...
            <TableRow key={batch.id}>
              ...
              <TableCell>
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-base font-semibold text-foreground leading-tight">{batch.tradeName}</p>
                    <p className="text-sm text-muted-foreground italic mt-0.5">{batch.genericName}</p>
                  </div>
                </div>
              </TableCell>
              ...
              {/* Qty */}
              <TableCell>
                <span className="text-base font-bold text-foreground">{batch.quantity.toLocaleString()}</span>
              </TableCell>
              ...
              <TableCell>
                <div className="flex flex-col gap-0.5">
                  ...
                  <span className={`text-xs font-medium ${isExpired ? "text-destructive" : days <= 30 ? "text-orange-500" : "text-muted-foreground"}`}>
              ...
              {/* Price */}
              <TableCell>
                <span className="text-base font-semibold text-foreground">${batch.purchasePrice.toFixed(2)}</span>
              </TableCell>

              {/* Status */}
              <TableCell>
                <span className={`inline-flex items-center rounded-md border px-2.5 py-1 text-sm font-semibold ${statusConfig[batch.status]}`}>
                  {batch.status}
                </span>
              </TableCell>
