...
const SupplierSearchInput: React.FC<Props> = ({
    ...
    ? suppliers
        .filter(s =>
          ...
          (s.name.toLowerCase().includes(debouncedQuery.toLowerCase()) ||
           s.company.toLowerCase().includes(debouncedQuery.toLowerCase()))
        )
        .slice(0, 8)
        .map(s => ({ supplierId: s.supplierId, name: s.name, company: s.company }))
    : [];
  ...
  const handleChange = (q: string) => {
    setQuery(q);
    if (q.trim().length >= 2) { updatePos(); setShowDropdown(true); }
    else setShowDropdown(false);
  };

  const handleSelect = (supplier: SupplierResult) => {
    onSelect(supplier);
    setQuery(`${supplier.name} — ${supplier.company}`);
    setShowDropdown(false);
  };
    ...
    <>
      <div className="relative">
        ...
        <input
          ...
          onFocus={() => { if (query.trim().length >= 2) { updatePos(); setShowDropdown(true); } }}
      ...
      {showDropdown && createPortal(
        ...
        >
          ...
          {showResults && filteredResults.map(s => (
            <button
              key={s.supplierId}
              type="button"
            ...
            >
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-0.5">{s.company}</p>
              </div>
            </button>
          ))}
          {showEmpty && (
            <div className="px-4 py-3">
              <p className="text-sm text-muted-foreground">No suppliers found for "{debouncedQuery}"</p>
            </div>
          )}
        </div>,
        document.body
